...
def make_group(cam_group_obj):
    for i, pic in enumerate(cam_group_obj[0].new_image_list):
        print("Groupe n°", i+1)
        pic_list = []
        for cam in cam_group_obj:
            #print(cam.name, cam.new_image_list[i].path)
            if cam.new_image_list[i].path == None:
                print("Il manque une image !!")
                pic_list = []
                break
            else:
                pic_list.append(cam.new_image_list[i].path)
        #print(pic_list)
        #create output dir
        if len(pic_list) > 0:
            output_dirname = os.path.basename(pic_list[0])[:20]
            output_path = os.path.abspath(os.path.join(os.path.dirname(pic_list[0]), os.pardir, output_dirname))
            try:
                os.mkdir(output_path)
            except FileExistsError:
                pass
            # copy or move pic to output directory
            for i, pic in enumerate(pic_list):
                #shutil.copy2(pic, os.path.join(output_path, os.path.basename(pic)))
                #shutil.copy2(pic, os.path.join(output_path, 'APN' + str(i) + ".jpg"))
                shutil.move(pic, os.path.join(output_path, 'APN' + str(i) + ".jpg"))


def log_from_timestamp(cam_obj):
    """ extract timestamp from a cam picture list
    :param cam_obj: cam_obj class
    :return: a list a log_infos namedtuple
    """
    loglist = []
    for i, pic in enumerate(cam_obj.image_list):
        loglist.append(log_infos(pic[1], "KTakepic", pic[1], 0, "111111", i))
    #print(loglist)
    return loglist
...
if __name__ == '__main__':
    ...
    # create logfile from the timestamps from the longest cam pic list
    pic_count_list = [cam.pic_count for cam in cam_group]
    cam_idx = pic_count_list.index(max(pic_count_list))
    loglist = log_from_timestamp(cam_group[cam_idx])

    cam_group.add_log(loglist)

    # Trying to correlate the shutter's timestamps with the images timestamps.
    for cam in cam_group:
        cam.new_image_list = correlate_log_and_pic(cam, auto=True)
    ...
    #import pdb; pdb.set_trace()
    print("=" * 80)
    make_group(cam_group)

    # DEBUG
    '''
    for cam in cam_group:
        print (cam.name)
        for pic in cam.image_list:
            print(pic)
        print ("new image list")
        for pic in cam.new_image_list:
            print(pic)
    '''
